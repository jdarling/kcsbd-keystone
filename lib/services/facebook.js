...
var credentials = {
	...
	callbackURL: production ? 'http://www.kcsbd.org/authentication/facebook?callback' : 'http://local.host:3000/authentication/facebook?callback'
};
...
exports.authenticateUser = function(req, res, next, callback)
{
	...
	var createSydjsUser = function() {
		...
		console.log('[services.facebook] - Creating user...' );
		console.log('------------------------------------------------------------');
		...
		console.log('[services.facebook] - user create data:', userData );
		
		// Create user
		data.sydjsUser = new User.model(userData);
		
		console.log('[services.facebook] - Created new instance of user.' );
		console.log('------------------------------------------------------------');
	...
	var saveSydjsUser = function() {
		...
		console.log('[services.facebook] - Saving user...' );
		console.log('------------------------------------------------------------');
		...
		console.log('[services.facebook] - user update data:', userData );
		...
		data.sydjsUser.save(function(err) {
			...
			if (err) {
				console.log(err);
				console.log("[services.facebook] - Error saving user.");
				console.log('------------------------------------------------------------');
				return callback(err);
				
			} else {
				
				console.log("[services.facebook] - Saved user.");
				console.log('------------------------------------------------------------');
	...
	var signinSydjsUser = function() {
		...
		console.log('[services.facebook] - Signing in user...');
		console.log('------------------------------------------------------------');
