...
var credentials = {
	...
	callbackURL: production ? 'http://www.kcsbd.org/authentication/twitter?callback' : 'http://local.host:3000/authentication/twitter?callback'
};
...
exports.authenticateUser = function(req, res, next, callback)
{
	...
	var createSydjsUser = function() {
		...
		console.log('[services.twitter] - Creating user...');
		console.log('------------------------------------------------------------');
		...
		console.log('[services.twitter] - user create data:', userData );
		
		// Create user
		data.sydjsUser = new User.model(userData);
		
		console.log('[services.twitter] - Created new instance of user.');
		console.log('------------------------------------------------------------');
	...
	var saveSydjsUser = function() {
		...
		console.log('[services.twitter] - Saving user...');
		console.log('------------------------------------------------------------');
		...
		console.log('[services.twitter] - user update data:', userData );
		...
		data.sydjsUser.save(function(err) {
			...
			if (err) {
				console.log(err);
				console.log("[services.twitter] - Error saving user.");
				console.log('------------------------------------------------------------');
				return callback(err);
				
			} else {
				
				console.log("[services.twitter] - Saved user.");
				console.log('------------------------------------------------------------');
	...
	var signinSydjsUser = function() {
		...
		console.log('[services.twitter] - Signing in user...');
		console.log('------------------------------------------------------------');
